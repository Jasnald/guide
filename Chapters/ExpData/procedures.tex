...
\begin{itemize}
    ...
    \item \texttt{fix\_rules} applies coordinate corrections via \texttt{DataTransformer} before fitting; see Section~\ref{subsec:core_transformer} for syntax.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Unexpected side IDs:} \texttt{TShapeParser.list\_ids} controls discovery — see Section~\ref{subsec:parser_t_shape}.
\end{itemize}
...
No outlier removal or segmentation occurs here for Exp2. The raw merged profile is saved as-is for manual inspection via the GUI before fitting. The averaging logic lives entirely inside \texttt{RecShapeParser} — see Section~\ref{subsec:parser_rec_shape}.
